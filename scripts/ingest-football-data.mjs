...
// API, 10 req/min) into src/data/leagues/{code}/{teams,matches,standings}.json,
// plus a src/data/competitions.json manifest (static metadata + each
// competition's current season/matchday from the API).
//
...
//
// Free tier is 10 req/min: 9 competitions x 2 calls (standings, matches) = 18
// calls, so each call is paced ~6.5s apart (a full pass takes ~2 minutes). A
// single competition's failure (e.g. season not yet loaded) is logged and
// skipped — it doesn't abort the rest of the run or touch that competition's
// last-good JSON on disk.
...
async function ingestCompetition(meta) {
  ...
  console.log(
    `[${code}] wrote ${teams.length} teams, ${standings.length} standings rows, ${matches.length} matches.`,
  );
